...
public class MazeImpl extends Maze implements Serializable, ClientListener, Runnable {
	 ...
	 */
	public static Maze readMazeFile(String mazefile) throws IOException, ClassNotFoundException {
	...
	// function for placing remote clients. It runs the same code as addClient,
	// but enables specifying position and direction instead of using random.
	public synchronized void addRemoteClient(RemoteClient client, Point point, Direction direction) {
		assert (client != null);
	...
	// moves a client to the specified point with the specified direction
	public boolean repositionClient(Client client, Point position, Direction dir) {
		boolean ret = false;
	...
	// to sync score, notifyFire is called by Mazewar.java.
	// Removed notification to be replaced by a packet.
	...
	// determine if moving forward would be a legal move
	public synchronized boolean checkMoveClientForward(Client client) {
		assert (client != null);
		Object o = clientMap.get(client);
		assert (o instanceof DirectedPoint);
		DirectedPoint dp = (DirectedPoint) o;
	...
	// determine if moving backwards would be a legal move
	public synchronized boolean checkMoveClientBackward(Client client) {
		assert (client != null);
		Object o = clientMap.get(client);
		assert (o instanceof DirectedPoint);
		DirectedPoint dp = (DirectedPoint) o;
...
//			update();
	...
	// provides access to notify client fired for signaling listeners
	public void notifyClientFiredPublic(Client c) {
		notifyClientFired(c);
	}
	...
	// interfaces to the notifyClientKilled function
	public void notifyKill(Client source, Client target) {
		notifyClientKilled(source, target);
	}
